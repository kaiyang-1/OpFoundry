...
template<class T> using v_ml_t = typename T::D_ACC;
...
template<class Traits> __device__ auto make_layout_rk(int lane_id);
template<class Traits> __device__ auto make_layout_rv(int lane_id);
template<class Traits> __device__ auto make_layout_o(int warp_id, int lane_id, int stride_o_h);
