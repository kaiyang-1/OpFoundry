...
// Declared in the per-variant kernel instantiation TU.
template<class Traits>
__global__ void pa_prefill_16mx8_32nx1_fp8_kernel(pa_fp8_kargs kargs);

// Launch wrapper — overloaded on the trait type so it selects its own kernel.
template<int Q, int KV, int D, int NW, class NOPE, class ROPE, class DO>
inline void pa_launch(pa_16mx8_32nx1_fp8_traits<Q, KV, D, NW, NOPE, ROPE, DO>,
                      const pa_fp8_kargs& kargs, dim3 grid, dim3 block) {
    pa_prefill_16mx8_32nx1_fp8_kernel<pa_16mx8_32nx1_fp8_traits<Q, KV, D, NW, NOPE, ROPE, DO>><<<grid, block>>>(kargs);
}
...
int run_pa_case(int H, int N, int total_pages, int total_tokens,
    ...
    printf("PA Prefill Attention: H_Q=%d, N=%d, D=%d, NoPE=fp8, RoPE=bf16, total_pages=%d, total_tokens=%d\n",
           H, N, PATraits::D_HEAD_SIZE, total_pages, total_tokens);
...
int main(int argc, char** argv) {
    ...
    return run_pa_case<pa_16mx8_32nx1_fp8_traits<16, 32, 640, 8, fp8_t, bf16_t, bf16_t>>(H, N, total_pages, total_tokens, verify, dense_kv);
}
