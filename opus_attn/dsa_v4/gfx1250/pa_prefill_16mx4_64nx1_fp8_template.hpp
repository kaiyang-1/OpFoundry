...
__device__ inline auto make_layout_rk_rope(int lane_id) {
    ...
    return opus::make_layout(
        k_block_shape,
        opus::unfold_x_stride(k_block_dim, k_block_shape, opus::tuple{opus::number<T::K_ROPE_ROW_LDS_ELEMS>{}, 1_I}),
        opus::unfold_p_coord(k_block_dim, opus::tuple{lane_id % T::W_N, lane_id / T::W_N}));
}
...
template<class T>
__device__ inline auto make_layout_rv_mxscl(int lane_id) {
    constexpr int MXSCL_BLOCK_SIZE = 32;

    constexpr auto k_block_shape = opus::make_tuple(
        opus::number<T::GEMM0_E_N>{},
        opus::number<T::W_N>{},
        opus::number<T::D_NOPE_PADDED_SIZE / MXSCL_BLOCK_SIZE>{});
...
__device__ __attribute__((always_inline)) void pa_prefill_accum_pipelined(pa_fp8_kargs kargs,
    ...
    const int seg_rot_rows = warp_id * T::W_N;
    auto stage_row  = [&](int j) { return (j * T::W_N + seg_rot_rows) & (T::KV_TILE_SIZE - 1); };
    ...
    auto u_rv_mxscl = make_layout_rv_mxscl<T>(lane_id);
    ...
    auto issue_kv_tile = [&](const u32x16_t& ids, int tile_idx) {
        const int wave_valid = valid_kv_len - (tile_idx * T::KV_TILE_SIZE + warp_id * T::ROWS_PER_WAVE);
        ...
        static_for<T::TDM_LOADS_PER_WAVE>([&](auto d) {
            ...
            static_for<T::INDICES_PER_TDM>([&](auto r) {
                ...
                const u32_t id = __builtin_amdgcn_readfirstlane(slot < wave_valid ? ids[slot] : (u32_t)kv_rows);
    ...
    auto compute_qk = [&]() __attribute__((always_inline)) {
        ...
        static_for<T::GEMM0_STAGE_N>([&](auto j) {
            ...
            clear(v_s_stages[j.value]);
            static_for<T::GEMM0_NOPE_E_K>([&](auto ek) {
                constexpr int sel = ek.value % 2;
                constexpr int dw  = ek.value / 2;
                v_s_stages[j.value] = mma0_nope.step_k(ek, v_k_nope, v_q_nope, v_s_stages[j.value],
                                                       scale_k[dw], scale_q[dw], number<sel>{}, number<sel>{});
            });
            v_s_stages[j.value] = mma0_rope(v_k_rope, v_q_rope, v_s_stages[j.value]);
            ...
            if constexpr (j.value == T::GEMM0_STAGE_N - 1) {
                ...
                constexpr index_t nope_blocks   = T::D_NOPE_SIZE / MXSCL_BLOCK_SIZE;          // only 14 carry data

                auto v_v_mxscl = load<mxscl_per_row>(s_k_nope[j.value], u_rv_mxscl + T::D_NOPE_SIZE);
                s_wait_dscnt(0_I);

                auto& scale_v = reinterpret_cast<vector_t<u32_t, mxscl_per_row / 4>&>(v_v_mxscl);
                auto* src     = reinterpret_cast<vector_t<u32_t, 2>*>(&v_k_nope);
                auto* dst     = reinterpret_cast<vector_t<D_ROPE, 8>*>(&v_v_nope);
                static_for<nope_blocks>([&](auto b) {
                    constexpr int dw  = b.value / 4;
                    constexpr int sel = b.value % 4;
                    dst[2 * b.value + 0] = __builtin_amdgcn_cvt_scale_pk8_bf16_fp8(src[2 * b.value + 0], scale_v[dw], sel);
                    dst[2 * b.value + 1] = __builtin_amdgcn_cvt_scale_pk8_bf16_fp8(src[2 * b.value + 1], scale_v[dw], sel);
                });
            }
        });
    };
    ...
    for (int tile_idx = 0; tile_idx < num_kv_tiles; ++tile_idx) {
        ...
        s_wait_kmcnt(0_I);
        issue_kv_tile(row_ids, tile_idx);
        ...
        // TODO: cast v_s to the bf16 P tile, V dequant, PV.
    }
}
